// -*- C++ -*-
//
// Package:    EcalValidationRelVal
// Class:      EcalValidationRelVal
...
//   float orbitNb = ev.orbitNumber();
  ...
  if(bad_event_ == false){
	...
	if ( ! recHitsEB.isValid() ) {
	  std::cerr << "EcalValidationRelVal::analyze --> recHitsEB not found" << std::endl; 
	}
	...
	if (! (ebDigis.isValid ()) ) {
	  std::cerr << "EcalValidationRelVal::analyze -->  ebDigis not found" << std::endl; 
	}
	...
	if ( ! recHitsEE.isValid() ) {
	  std::cerr << "EcalValidationRelVal::analyze --> recHitsEE not found" << std::endl; 
	}
	...
	if (! (eeDigis.isValid ()) ) {
	  std::cerr << "EcalValidationRelVal::analyze -->  eeDigis not found" << std::endl; 
	}
